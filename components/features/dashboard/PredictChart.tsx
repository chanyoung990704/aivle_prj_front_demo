...
const CustomTooltip = ({ active, payload, label, unit }: any) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-white/95 backdrop-blur-sm border border-line p-4 rounded-xl shadow-soft">
...
export default function PredictChart({
    ...
    return (
        <div className="bg-white rounded-3xl p-6 border border-line shadow-card hover:shadow-soft transition-shadow duration-300">
            <div className="flex justify-between items-start mb-8">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <span className="bg-paper px-3 py-1 rounded-full text-xs font-bold text-accent tracking-wide uppercase">
                            {metricCode}
                        </span>
                ...
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <XAxis
                            ...
                            axisLine={false}
                            tickLine={false}
                            tick={{ fill: '#6f7484', fontSize: 12, fontFamily: 'var(--font-space-grotesk)' }}
                            dy={10}
                        />
                        <YAxis
                            axisLine={false}
